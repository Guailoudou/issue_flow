...
export function resolveSafeRedirectTarget(from: unknown): string {
  let path = '';
...
export function LoginPage() {
  ...
  const mutation = useMutation({
    mutationFn: async (values: LoginValues) =>
      (await api<{ user: User }>('/auth/login', { method: 'POST', ...jsonBody(values) })).user,
    onSuccess: (value) => {
      setSessionUser(queryClient, value);
  ...
  return (
    <AuthPageLayout
      ...
      platformDescription={settings.description}
      title="欢迎回来"
      description="登录以继续处理 Issue"
      footer={
        <>
          还没有账号？{' '}
          <Link
            className="font-semibold text-brand-700 underline-offset-2 hover:underline focus-visible:rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600"
            to="/register"
          >
            使用邀请码注册
          </Link>
      ...
      <form onSubmit={handleSubmit((values) => mutation.mutate(values))} className="mt-7 space-y-5">
        ...
        <FormField label="用户名" htmlFor="username" error={errors.username?.message}>
          <div className="relative">
            <UserRound className="absolute left-3 top-3.5 size-4 text-slate-400" aria-hidden="true" />
            <Input
              id="username"
        ...
        <FormField label="密码" htmlFor="password" error={errors.password?.message}>
          <div className="relative">
            <LockKeyhole className="absolute left-3 top-3.5 size-4 text-slate-400" aria-hidden="true" />
            <Input
              id="password"
              type="password"
