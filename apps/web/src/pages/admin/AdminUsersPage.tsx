...
export function AdminUsersPage() {
  ...
  return <>
    ...
    <div className="surface overflow-hidden">
      {query.isPending ? <Spinner /> : query.isError ? <ErrorPanel message={query.error.message} onRetry={() => void query.refetch()} /> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-50 text-slate-600"><tr><th className="px-4 py-3 font-semibold">用户</th><th className="px-4 py-3 font-semibold">邮箱</th><th className="px-4 py-3 font-semibold">角色</th><th className="px-4 py-3 font-semibold">状态</th><th className="px-4 py-3 text-right font-semibold">操作</th></tr></thead><tbody className="divide-y">{query.data.map((user) => <tr key={user.id}><td className="px-4 py-3"><div className="flex items-center gap-2"><Avatar name={user.displayName} size="sm" /><div><p className="font-medium">{user.displayName}</p><p className="text-xs text-slate-500">@{user.username}</p></div></div></td><td className="px-4 py-3 text-slate-600">{user.email || '—'}</td><td className="px-4 py-3">{user.role === 'ADMIN' ? '管理员' : '用户'}</td><td className="px-4 py-3"><span className={user.active ? 'text-emerald-700' : 'text-red-700'}>{user.active ? '已启用' : '已停用'}</span></td><td className="px-4 py-3"><div className="flex justify-end gap-1"><Button variant="ghost" disabled={user.id === currentUser?.id} title={user.id === currentUser?.id ? '不能在此修改自己的用户名' : undefined} icon={<Pencil className="size-4" aria-hidden="true" />} onClick={() => openRename(user)}>编辑用户名</Button><Button variant="ghost" icon={<KeyRound className="size-4" aria-hidden="true" />} onClick={() => setResetUser(user)}>重置密码</Button><Button variant="ghost" disabled={user.role === 'ADMIN'} loading={toggle.isPending && toggle.variables?.id === user.id} icon={user.active ? <UserX className="size-4" aria-hidden="true" /> : <UserCheck className="size-4" aria-hidden="true" />} onClick={() => toggle.mutate(user)}>{user.active ? '停用' : '启用'}</Button></div></td></tr>)}</tbody></table></div>}
    </div>
