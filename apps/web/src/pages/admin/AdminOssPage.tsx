...
const initialForm = { enabled: false, endpoint: '', region: '', bucket: '', prefix: 'issueflow/attachments', forcePathStyle: false, accessKeyId: '', accessKeySecret: '', clearCredentials: false };
...
export function AdminOssPage() {
  ...
  useEffect(() => {
    ...
    setForm({ ...query.data.setting, accessKeyId: '', accessKeySecret: '', clearCredentials: false });
  }, [query.data]);
  const save = useMutation({
    mutationFn: () => api<OssSettingResponse>('/admin/storage/oss', { method: 'PUT', ...jsonBody({
      enabled: form.enabled, endpoint: form.endpoint.trim(), region: form.region.trim(), bucket: form.bucket.trim(), prefix: form.prefix.trim(), forcePathStyle: form.forcePathStyle, clearCredentials: form.clearCredentials,
    ...
    onSuccess: async () => { setForm((value) => ({ ...value, accessKeyId: '', accessKeySecret: '', clearCredentials: false })); await query.refetch(); },
  });
  ...
  const valid = form.endpoint.trim() && form.bucket.trim() && form.prefix.trim();

  return <>
    <PageHeader title="S3 兼容存储" description="连接 AWS S3、阿里云 OSS、MinIO 等 S3 兼容对象存储" />
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      <form className="surface min-w-0 space-y-5 p-5" onSubmit={(event) => { event.preventDefault(); if (!form.clearCredentials || window.confirm('确认清除已保存的 AccessKey ID 和 AccessKey Secret？')) save.mutate(); }}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <FormField label="Region" htmlFor="oss-region" hint="可选；留空时使用 us-east-1"><Input id="oss-region" value={form.region} onChange={(event) => setForm({ ...form, region: event.target.value })} /></FormField>
          ...
          <FormField label="AccessKey ID" htmlFor="oss-access-key-id" hint={setting.hasAccessKeyId ? '已配置；留空将保留原值' : undefined}><Input id="oss-access-key-id" type="password" autoComplete="new-password" disabled={form.clearCredentials} value={form.accessKeyId} onChange={(event) => setForm({ ...form, accessKeyId: event.target.value })} /></FormField>
          <FormField label="AccessKey Secret" htmlFor="oss-access-key-secret" hint={setting.hasAccessKeySecret ? '已配置；留空将保留原值' : undefined}><Input id="oss-access-key-secret" type="password" autoComplete="new-password" disabled={form.clearCredentials} value={form.accessKeySecret} onChange={(event) => setForm({ ...form, accessKeySecret: event.target.value })} /></FormField>
          <label className={`flex min-h-11 items-center gap-3 rounded-lg border border-red-200 bg-red-50 p-3 text-red-900 transition-colors md:col-span-2 ${form.enabled || (!setting.hasAccessKeyId && !setting.hasAccessKeySecret) ? 'cursor-not-allowed opacity-60' : 'cursor-pointer hover:bg-red-100'}`}><Checkbox checked={form.clearCredentials} disabled={form.enabled || (!setting.hasAccessKeyId && !setting.hasAccessKeySecret)} onChange={(event) => setForm({ ...form, clearCredentials: event.target.checked, accessKeyId: '', accessKeySecret: '' })} /><span><span className="block text-sm font-semibold">清除已保存的 AccessKey 凭据</span><span className="block text-xs">请先关闭 S3；保存时会再次确认，清除后不可恢复。</span></span></label>
        </div>
