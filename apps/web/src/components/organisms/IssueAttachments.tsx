...
export function IssueAttachments({ issueId }: { issueId: number }) {
  ...
  return <section className="surface overflow-hidden" aria-labelledby="issue-attachments-title">
    ...
    <div className="space-y-4 p-4">
      ...
      {query.isPending ? <Spinner label="正在加载附件" /> : query.isError ? <ErrorPanel message={query.error.message} onRetry={() => void query.refetch()} /> : attachments.length === 0 ? <EmptyState icon={Paperclip} title="暂无附件" description="可以在上方选择文件并上传。" /> : <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3" aria-label="已上传附件">
