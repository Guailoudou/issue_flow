...
export interface IssueFormValues { title: string; body: string; assigneeIds: number[]; labelIds: number[]; milestoneId: number | ''; attachments: File[] }
export function IssueForm({ users, labels, milestones, initial, loading, error, submitLabel = '创建 Issue', onSubmit }: { users: User[]; labels: Label[]; milestones: Milestone[]; initial?: Partial<IssueFormValues>; loading?: boolean; error?: string; submitLabel?: string; onSubmit: (values: IssueFormValues) => void }) {
  const { register, handleSubmit, control, formState: { errors } } = useForm<IssueFormValues>({ defaultValues: { title: '', body: '', assigneeIds: [], labelIds: [], milestoneId: '', attachments: [], ...initial } });
